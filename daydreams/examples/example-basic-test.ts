...
async function main() {
    ...
    const llmClient = new LLMClient({
        model: "anthropic/claude-3-5-sonnet-latest", // High performance model
    });
    ...
    // Use a simple mock memory instead
    const memory = {
        search: async () => [],
    ...
    // Comment out all ChromaDB operations
    // await memory.purge();
    ...
    // Initialize the main reasoning engine with explicit action types
    const dreams = new ChainOfThought(
        llmClient,
        memory,
        {
            worldState: `IMPORTANT: You must use uppercase "RESPONSE" for all responses, like this exact format:
{
    "type": "RESPONSE",
...
Available action types (MUST BE UPPERCASE):
- RESPONSE: For general responses
- EXECUTE_TRANSACTION: For blockchain transactions
- GRAPHQL_FETCH: For querying data`
        },
        {
    ...
    dreams.registerOutput({
        ...
        role: HandlerRole.OUTPUT,
        execute: async (data: any) => {
            // Transform incoming action type to uppercase
            if (data.action) {
                data.action.type = data.action.type.toUpperCase();
            }
            console.log("DEBUG: Executing RESPONSE handler with data:", data);
    ...
    while (true) {
        ...
        try {
            ...
            const result = await dreams.think(userInput) as unknown as { content?: string };
            console.log(chalk.green("\n✨ Task completed successfully!"));
            if (result?.content) {
                console.log(chalk.blue("Response:"), result.content);
            }
        } catch (error) {
